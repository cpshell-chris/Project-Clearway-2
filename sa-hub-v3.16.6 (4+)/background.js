// Background service worker for Service Advisor Hub Extension
// Handles both Knowledge Assistant and VCA Generator
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'sah_openSidePanel') {
    const tabId    = sender.tab?.id;
    const windowId = sender.tab?.windowId;
    console.log('[Hub] openSidePanel received, tabId:', tabId, 'windowId:', windowId);
    if (tabId) {
      chrome.sidePanel.open({ tabId }, () => {
        if (chrome.runtime.lastError) {
          console.error('[Hub] sidePanel.open error:', chrome.runtime.lastError.message);
          // Fallback: try windowId
          if (windowId) chrome.sidePanel.open({ windowId });
        } else {
          console.log('[Hub] Side panel opened successfully');
        }
      });
    }
  ...
  if (request.action === 'sah_searchKnowledge') {
  ...
  if (request.action === 'sah_generateVCA') {
...
async function performKnowledgeSearch(query, apiKey, vehicleContext) {
  ...
  // Build vehicle + history block if RO data was passed in
...
// VCA instructions embedded (simplified version - full instructions would be loaded from file in production)
// Complete VCA Instructions v3.1d-R10 for Service Advisor Hub
// This is the full VCA system that will replace the simplified version in background.js

// Complete VCA Instructions - Canonical Version from VCA_INSTRUCTIONS.md
...
**Brakes:**
✓ Front pads: [measurement/condition]
✓ Rear pads: [measurement/condition]
✗ [Any failed/needs attention item]
⚠ [Any advisory item]

**Tires:**
✓ Tread depth: [measurements]
✓ Tire pressure: [PSI readings]
✓ Wear pattern: [condition]
...
4. Complete DVI Summary by system with ✓/✗/⚠ symbols
...
// Listen for TekMetric messages
// TM handlers merged into main listener above

// ── Test connection to Cloud Run ────────────────────────────────────
async function tmTestConnection(creds) {
  const baseUrl = creds.cloudRunUrl || creds.environment;
  
  console.log('[TekMetric] Testing Cloud Run connection:', baseUrl);
  
  // Try a simple health check or test RO fetch
  const testEndpoints = [
    `${baseUrl}/health`,
    `${baseUrl}/`,
    `${baseUrl}/repair-order/528446`, // Known test RO
  ];

  for (const url of testEndpoints) {
    try {
      console.log('[TekMetric] Trying:', url);
      const res = await fetch(url);
      console.log('[TekMetric] Response status:', res.status);
      
      if (res.ok) {
        console.log('[TekMetric] ✓ Cloud Run accessible');
        return { 
      ...
      console.log('[TekMetric] Connection attempt failed:', err.message);
    }
  }
...
// ── Fetch single RO by number via Cloud Run ────────────────────────
async function tmGetRepairOrder(roNumber, creds) {
  const baseUrl = creds.cloudRunUrl || creds.environment;
  
  console.log('[TekMetric] Fetching RO:', roNumber, 'from', baseUrl);
  ...
  for (const url of endpointPatterns) {
    try {
      ...
      if (!res.ok) {
        ...
        console.log('[TekMetric] Error response:', text);
        continue;
      }

      const data = await res.json();
      console.log('[TekMetric] ✓ Successfully fetched RO data');
      
      // Format the response for VCA
      return tmFormatROData(data);
      
    } catch (err) {
      console.log('[TekMetric] Endpoint failed:', err.message);
    }
  }
...
// ── Search (simplified for Cloud Run) ──────────────────────────────
async function tmSearch(query, searchType, creds) {
  const baseUrl = creds.cloudRunUrl || creds.environment;
  
  console.log('[TekMetric] Search:', searchType, query);
  
  // For now, just try to fetch as RO number
  if (searchType === 'ro') {
    return tmGetRepairOrder(query, creds);
  }
  
  // Customer/phone search would need specific Cloud Run endpoints
  throw new Error('Customer search not yet implemented. Use RO number search.');
}

// ── Format RO data for VCA ──────────────────────────────────────────
function tmFormatROData(data) {
  console.log('[TekMetric] Formatting data:', data);
  console.log('[TekMetric] Odometer fields — ro.odometerIn:', (data.repairOrder||data.data||data).odometerIn, '| ro.mileage:', (data.repairOrder||data.data||data).mileage, '| vehicle.mileage:', (data.repairOrder||data.data||data).vehicle?.mileage);
  
  // Handle different possible response formats from Cloud Run
  ...
  // Jobs / services — extract every possible note/finding field
  const jobLines = jobs.map((job, i) => {
    ...
    // Collect all technician note fields TekMetric may use
    const noteFields = [
      job.technicianNote,
      job.laborNote,
      job.note,
    ...
    // Findings array (DVI-style per-job findings)
  ...
  // DVI / inspection items — capture all rating/severity levels
...
async function swGeneratePMRR(roData, vehicleHistory, apiKey) {
  ...
  // estimatedMileage is always currentMileage + 6,000 (the service target)
  const estimatedMileage = currentMileage + 6000;

  // recommendedInterval = how many months until 6,000 miles are driven
  // Uses avgMilesPerDay from vehicle history if available; defaults to 1,000 mi/month
  let defaultInterval = 6; // default: 1,000 mi/month → 6 months
  if (vehicleHistory?.avgMilesPerDay && vehicleHistory.avgMilesPerDay > 0) {
    ...
    // Clamp between 1 and 18 months for sanity
    defaultInterval = Math.max(1, Math.min(18, Math.round(monthsTo6000)));
    console.log(`[PMRR] avgMilesPerDay=${vehicleHistory.avgMilesPerDay.toFixed(2)}, milesPerMonth=${milesPerMonth.toFixed(0)}, interval=${defaultInterval} months`);
  } else {
    console.log('[PMRR] No vehicle history — defaulting to 6 months / 1,000 mi/month');
  }

  const defaultMileage = estimatedMileage;

  // Build vehicle history summary for the PMRR prompt
...
4. STEP D: Does not apply. "no-history" is now correct for ALL services with no record, regardless of whether the vehicle has other history.
...
{"recommendedInterval":${defaultInterval},"estimatedMileage":${defaultMileage},"services":[{"name":"service name","category":"overdue|essential|recommended|no-history"}]}`;
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      // Strip any oil service or tire rotation the AI included (injected deterministically below)
      parsed.services = (parsed.services || []).filter(s => {
        const n = (s.name || '').toLowerCase();
        return !n.includes('oil') && n !== 'tire rotation';
      });

      // Inject oil service and tire rotation as recommended at the top — always
