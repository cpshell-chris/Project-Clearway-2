// Service Advisor Hub - Unified JavaScript
// Handles both Knowledge Assistant and VCA Generator

// ==================== INITIALIZATION ====================

window.addEventListener('load', () => {
    console.log('Service Advisor Hub v3.16 loaded');
    ...
    document.getElementById('hub-tm-save-btn')?.addEventListener('click', () => {
        ...
        chrome.runtime.sendMessage({ action: 'sah_tmTestConnection', credentials: creds }, response => {
            if (response?.success) {
...
const TOOL_META = {
    ...
    sw:  { icon: '🪄', name: 'Scheduling Wizard',          panel: 'tool-sw-content',  hiw: { title: '🪄 Scheduling Wizard', body: '<p style="font-size:12px;color:#4a5568;line-height:1.65;margin-bottom:10px;">Books the next appointment directly into TekMetric in three steps: select date and time, choose services, then confirm.</p><p style="font-size:12px;color:#4a5568;line-height:1.65;margin-bottom:10px;">When an RO is loaded, the Wizard uses the vehicle\'s service history to calculate the recommended interval and pre-populate the Preventive Maintenance and Repairs Recommendation predictive maintenance list.</p><p style="font-size:12px;color:#4a5568;line-height:1.65;">Appointments are created with the title format: Scheduling Wizard - [Customer Name] - [Year Make Model].</p>' } },
    vca: { icon: '📋', name: 'Vehicle Consultant Advisor', panel: 'tool-vca-content', hiw: { title: '📋 Vehicle Consultant Advisor', body: '<p style="font-size:12px;color:#4a5568;line-height:1.65;margin-bottom:10px;">Paste TekMetric data to generate three report types: Customer Notes, Service Advisor Reports, and Exit Schedule Reports.</p><p style="font-size:12px;color:#4a5568;line-height:1.65;">Powered by Cardinal Plaza Shell\'s Preventive Maintenance and Repairs Recommendation engine with T0 (today), T6 (6 months), and T12 (12 months) predictive time windows.</p>' } },
    ama: { icon: '💬', name: 'Answering Customer Questions', panel: 'tool-ama-content', hiw: { title: '💬 Answering Customer Questions', body: '<p style="font-size:12px;color:#4a5568;line-height:1.65;margin-bottom:10px;"><strong>Customer Question mode:</strong> In-person or phone — answer questions, overcome objections, and help customers decide. Vehicle context from the loaded RO is automatically included.</p><p style="font-size:12px;color:#4a5568;line-height:1.65;">Use the length and detail chips to refine, or type a follow-up in the chat thread to adjust tone, depth, or emphasis.</p>' } },
};
...
function openTool(toolKey) {
    ...
    // Show correct panel
    ['tool-ka-content','tool-sw-content','tool-vca-content','tool-ama-content','tool-sod-content'].forEach(id => { const el = document.getElementById(id); if(el) el.style.display='none'; });
...
// ==================== API KEY MANAGEMENT ====================
// API key is now managed through the Hub Settings modal (⚙️ gear in header)
...
function kaPopulate(r) {
    ...
    if (urgencyEl) {
        ...
        const icons = { 'IMMEDIATE': '🔴', 'RECOMMENDED': '🟠', 'PREVENTIVE CARE': '🟢' };
        const labels = { 'IMMEDIATE': 'Immediate Attention', 'RECOMMENDED': 'Recommended', 'PREVENTIVE CARE': 'Preventive Care' };
        urgencyEl.innerHTML = `<div style="font-size:13px;font-weight:700;color:#7A3520;margin-bottom:4px;">${icons[lvl] || '🟢'} ${labels[lvl] || lvl}</div><div style="font-size:12px;color:#495057;line-height:1.5;">${escapeHTML(r.urgency_note || '')}</div>`;
    }
...
async function searchKnowledge() {
    ...
    chrome.storage.local.get(['apiKey'], async (storage) => {
        if (!storage.apiKey) {
            alert('No API key found. Tap ⚙️ in the header to add your key.');
            return;
        }
        ...
        chrome.runtime.sendMessage(
            { action: 'sah_searchKnowledge', query, apiKey: storage.apiKey, vehicleContext },
            (response) => {
...
async function generateVCA(type) {
    ...
    chrome.storage.local.get(['apiKey'], async (storage) => {
        ...
        if (!apiKey) {
            ...
            const result     = document.getElementById('vca-result');
            if (resultText) resultText.textContent = 'API key required. Tap ⚙️ in the header to add your Claude API key.';
        ...
        try {
            chrome.runtime.sendMessage(
                { action: 'sah_generateVCA', input: input, type: type, apiKey: apiKey },
                (response) => {
...
function sodShowView(viewId) {
    document.querySelectorAll('#sod-content .sod-view').forEach(v => v.classList.remove('active'));
...
async function sodCallAPI(systemPrompt, messages) {
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(['apiKey'], async (result) => {
            ...
            chrome.runtime.sendMessage(
                { action: 'sah_sodCall', systemPrompt, messages, apiKey },
                (response) => {
...
async function amaGenerateCustomer() {
    ...
    try {
        ...
        console.error('[AMA Customer Error]', err);
    }
...
async function amaGenerateReview() {
    ...
    try {
        ...
        alert('AMA Error: ' + (err?.message || String(err)));
        console.error('[AMA Review Error]', err);
    }
...
async function amaCallAPI(systemPrompt, messages) {
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(['apiKey'], (result) => {
            ...
            chrome.runtime.sendMessage(
                { action: 'sah_amaCall', systemPrompt, messages, apiKey },
                (response) => {
                    if (chrome.runtime.lastError) { reject(new Error(chrome.runtime.lastError.message)); return; }
                    if (response?.success) resolve(response.data);
                    else reject(new Error(response?.error || 'Unknown error'));
...
function checkTabURL(url) {
    ...
    if (match) {
        ...
        if (roId !== lastRoId) {
            lastRoId = roId;
            console.log('[TekMetric] Detected RO ID in URL:', roId);
            tmAutoFetchRO(roId);
        }
...
async function tmAutoFetchRO(roId) {
    console.log('[TekMetric] Auto-fetching RO:', roId);
    
    chrome.runtime.sendMessage(
        { action: 'sah_tmGetRO', roNumber: roId, credentials: tmCredentials },
        response => {
            if (response?.success) {
                tmLoadRO(response.data);
            } else {
                console.error('[TekMetric] Auto-fetch failed:', response?.error);
...
async function tmSaveAndTest() {
    ...
    chrome.runtime.sendMessage(
        { action: 'sah_tmTestConnection', credentials: creds },
        response => {
            if (response?.success) {
                chrome.storage.local.set({ tmCredentials: creds });
                tmCredentials = creds;
...
function tmTestConnection() {
    if (!tmCredentials) return;
    chrome.runtime.sendMessage(
        { action: 'sah_tmTestConnection', credentials: tmCredentials },
        response => {
            if (response?.success) {
...
async function swLoadRO(roId) {
    ...
    chrome.storage.local.get(['tmCredentials','apiKey'], async result => {
        ...
        try {
            const roResp = await swSend({ action:'sah_swFetchRO', roId, cloudRunUrl: creds.cloudRunUrl });
            ...
            const histResp = await swSend({ action:'sah_swFetchVehicleHistory', vehicleId: swRoData.vehicle.id, shopId: swRoData.shopId, cloudRunUrl: creds.cloudRunUrl });
            if (histResp.success) {
                swVehicleHistory = histResp.data;
                console.log('[SW] Vehicle history:', swVehicleHistory);
            } else {
                console.log('[SW] Vehicle history fetch failed:', histResp.error);
            }

            const pmrrResp = await swSend({ action:'sah_swGeneratePMRR', data: swRoData, vehicleHistory: swVehicleHistory, apiKey });
...
async function swLoadScheduleData() {
    ...
    chrome.storage.local.get(['tmCredentials'], async result => {
        ...
        try {
            const cr = await swSend({ action:'sah_swFetchAppointmentCounts', shopId: swRoData.shopId,
...
async function swConfirmAppointment() {
    ...
    chrome.storage.local.get(['tmCredentials'], async result => {
        const creds = result.tmCredentials;
        if (!creds?.cloudRunUrl) { alert('TekMetric not configured. Tap ⚙️ to set up.'); btn.disabled=false; btn.textContent='Confirm Appointment'; return; }
        try {
            const resp = await swSend({ action:'sah_swCreateAppointment', appointmentData: apptData, cloudRunUrl: creds.cloudRunUrl });
            if (resp.success) {
...
function swOpenScheduler(e) {
    ...
    chrome.storage.local.get(['tmCredentials'], result => {
        const creds = result.tmCredentials;
        if (!creds?.shopId) { alert('Shop ID not configured. Tap ⚙️ to set up.'); return; }
...
async function mocSend() {
  ...
  if (!key) {
    mocRemoveTyping();
    document.getElementById('moc-answer-text').textContent = 'API key not found. Please configure it in Hub Settings (⚙️).';
